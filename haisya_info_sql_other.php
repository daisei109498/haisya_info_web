...
//		$lng_max = intval(round((strval($row6['Lng'])+0.005)*100));
	...
	$sql = 'SELECT haisya_info.TodokeCd, haisya_info.todoke,haisya_info.driver, haisya_info.suryo,max(haisya_info.Lat) as Lat_max,max(haisya_info.Lng) as Lng_max FROM haisya_info GROUP BY haisya_info.TodokeCd, haisya_info.driver, haisya_info.suryo HAVING (((haisya_info.TodokeCd) Is Not Null) AND ((haisya_info.suryo)<>"分納"))';
	$statement = $link -> query($sql);
